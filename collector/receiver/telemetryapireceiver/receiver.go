...
type telemetryAPIReceiver struct {
	...
	logger                *zap.Logger // queue is a synchronous queue and is used to put the received log events to be dispatched later
	queue                 *queue.Queue
...
func (r *telemetryAPIReceiver) Start(_ context.Context, host component.Host) error {
	address := listenOnAddress()
	r.logger.Info("Listening for requests", zap.String("address", address))
	r.httpServer = &http.Server{Addr: address}
	http.HandleFunc("/", r.httpHandler)
	go func() {
...
func (r *telemetryAPIReceiver) createPlatformInitSpan(start, end string) (ptrace.Traces, error) {
	...
	ss.Scope().Attributes().PutStr("instrumentation.library", "telemetryapi")
	span := ss.Spans().AppendEmpty()
	span.SetSpanID(newSpanID())
	span.SetName("platform.initRuntimeDone")
	span.SetKind(ptrace.SpanKindInternal)
	span.Attributes().PutBool("faas.coldstart", true)
